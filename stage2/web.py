...
def state(session):
    ...
    return {
        ...
        "longterm": LONGTERM.recall(work.customer_email, getattr(work, "session_id", None)),
    }
...
<div class="col" id="left">
  <header><b>Ami</b><span>__MODEL__</span>
    <select id="planner" title="Planning strategy" style="margin-left:10px;background:#222733;
            color:var(--txt);border:1px solid var(--line);border-radius:6px;padding:4px 8px">
      <option value="react">ReAct — step by step</option>
      <option value="plan">Plan-and-execute — plan first</option>
    </select>
...
<div class="col" id="right">
  ...
  <div id="panel">
    ...
    <div id="wm">(empty)</div>
    <h3 style="margin-top:14px">Long-term memory — this customer</h3>
    <div id="ltm" class="empty" style="font-size:12px">Not identified yet.</div>
  </div>
</div>
...
function draw(d) {
  ...
  $('ltm').textContent = d.longterm || 'First conversation with this customer.';
  $('ltm').className = d.longterm ? '' : 'empty';
...
async function send(e) {
  ...
  try {
    const r = await fetch('/chat', {method:'POST',
      body: JSON.stringify({message:text, planner: $('planner').value})});
